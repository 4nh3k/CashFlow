...
class LocalStorageRepository<T extends { id: string }> {
  ...
  private getAll(): T[] {
    const data = storageManager.get<T>(this.key);
    // Process data to convert ISO strings back to Date objects where needed
    return data.map(item => this.processDataFromStorage(item));
  }

  // Helper method to process data from storage (convert ISO strings back to Dates)
  private processDataFromStorage(item: any): T {
  ...
  private saveAll(items: T[]): void {
    // Convert items to storage format (with ISO strings for dates)
    const storageItems = items.map(item => this.processDataForStorage(item));
    storageManager.set(this.key, storageItems);
  }

  async create(data: Omit<T, 'id' | 'createdAt' | 'updatedAt'>): Promise<T> {
    try {
      // Get items and ensure it's a mutable array
      const existingItems = this.getAll();
      const items = Array.isArray(existingItems) ? [...existingItems] : [];
      
      // Ensure dates are properly handled for storage
      ...
      // Use push to add the new item
      items.push(newItem);
      this.saveAll(items);
      
      // Return the item with proper Date objects
      return this.processDataFromStorage(newItem);
    } catch (error) {
      console.error(`[LocalStorageRepository] Create error for ${this.key}:`, error);
      throw new RepositoryError(`Failed to create item: ${error instanceof Error ? error.message : 'Unknown error'}`, 'CREATE_ERROR');
    }
  }

  // Helper method to process data for storage (convert Dates to ISO strings)
  ...
  async findAll(filter?: Record<string, any>): Promise<T[]> {
    try {
      ...
      console.error(`[LocalStorageRepository] findAll error:`, error);
  ...
  async update(id: string, data: Partial<T>): Promise<T | null> {
    try {
      const items = [...this.getAll()]; // Create a mutable copy
