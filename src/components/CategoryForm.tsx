...
const CategoryForm: React.FC<CategoryFormProps> = ({
  ...
  const checkNameUniqueness = async (name: string): Promise<boolean> => {
    try {
      const existingCategory = await enhancedCategoryRepository.findByName(name);
  ...
  const onFormSubmit = async (data: CategoryFormData) => {
    try {
      ...
      if (isEditMode && category) {
        ...
        const updatedCategory = await enhancedCategoryRepository.update(category.id, updateData);
        if (!updatedCategory) {
        ...
        // Create new category
        const createData: Omit<Category, 'id' | 'createdAt' | 'updatedAt'> = {
          name: data.name,
          defaultType: data.defaultType,
          isDefault: false,
          ...(data.color && { color: data.color }),
        ...
        result = await enhancedCategoryRepository.create(createData);
      }

      console.log(`Category ${isEditMode ? 'updated' : 'created'} successfully:`, result);
    ...
    <div className="max-w-md mx-auto bg-white rounded-lg border border-gray-200 shadow-sm p-6">
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
            ...
            render={({ field }) => (
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                  errors.name || nameError ? 'border-red-300' : 'border-gray-300'
                }`}
              />
            )}
          />
        ...
        <div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="grid grid-cols-2 gap-3">
                <label className={`relative flex items-center justify-center p-3 border rounded-lg cursor-pointer ${
                  field.value === 'expense' 
          ...
          />
          {errors.defaultType && (
            <p className="mt-1 text-sm text-red-600">{errors.defaultType.message}</p>
          )}
        </div>
